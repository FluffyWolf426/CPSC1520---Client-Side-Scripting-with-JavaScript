...
{"bookId":1,
  ...
  "author": "J.K. Rowling/Mary GrandPao",
  "rating":4.57,
  "isbn":"439785960",
  "language":"eng",
  "numberOfRatings": 652}
...
const getAuthors = (bookList) => {
  // get all of the authors but it's not going to be unique
  let authorsOnly = bookList.map((book) => {
    return book.author
  })
  // let's get the distinct authors
  authorsOnly = authorsOnly.filter((author, index)=> {
    // is the current index, the first index of this instance.
    return authorsOnly.indexOf(author) === index
    // is the same as doing this a bit more explicitly
    // if (authorsOnly.indexOf(author) === index) {
    //   return true
    // } else {
    //   return false
    // }
...
bookFilterForm.addEventListener("submit", (event)=> {
  ...
  let author = bookFilterForm.elements['author']
   // check to see if the input of title is in the array
  if (query.value !== "") {
    bookSubset = bookSubset.filter((book)=> {
      // if the book.title contains the substring query.value
      let titleInput = query.value.toLowerCase().trim()
      let title = book.title.toLowerCase()
      return title.includes(titleInput)
    })
  }
  // we'll do the same for author select
  if (author.value !== "") {
    bookSubset = bookSubset.filter((book)=> {
      // if the book.author contains the substring
      let authorInput = author.value // what the user has selected
      // are they the same?
      return book.author === authorInput
    })
  }
  console.log(bookSubset)
  // render those books.
...
// render all of the authors to the select
renderAuthorOptions()
